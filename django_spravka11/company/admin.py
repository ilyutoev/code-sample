...
from company.models import Category, Company, Address, Phone, City, CompanyImage,\
    CompanyNews, CompanyService, Timetable, CompanyPage, SeoCategory
...
class AddressAdmin(ImportExportModelAdmin, admin.ModelAdmin):
    ...
    filter_horizontal = ('category', 'seo_category')


class CompanyNewsAdmin(admin.ModelAdmin):
    raw_id_fields = ('company',)
    autocomplete_lookup_fields = {'fk': ['company']}
...
class SeoCategoryAdmin(MPTTModelAdmin):
    search_fields = ['name']
...
admin.site.register(SeoCategory, SeoCategoryAdmin)
