...
def unpack_line(line):
    ...
    file_price = line_list[3]
    return file_name, file_date, file_time, file_price
...
def main():
    ...
    for i, line in enumerate(lines):
        if not line.find("#") > -1:
            ...
            file_name, file_date, file_time, file_price = unpack_line(line)
            ...
            if i == 1: #функцию вызываем только один раз
                exist_date_event = last_date_event('renova', date.strftime("%Y-%m-%d")) #выгружаем последни даты из базы
